...
def on_message(self, msg):
    ...
    if "results" in data:
        ...
        final_string=data['results'][0]['alternatives'][0]['transcript']
...
def on_close(ws):
    ...
    if LAST:
        FINALS.append(LAST)
    transcript=""
    transcript = "".join([x['results'][Counter]['alternatives'][Counter]['transcript']
                          for x in FINALS])
    print(transcript+"<<<")
...
def main():
    assistant_apikey = 'dbzq11fbXsd3rf-Ox-dHlh_1hOZlRMhB3HoNh8RhfILK'
    assistant_url = 'https://api.eu-gb.assistant.watson.cloud.ibm.com'
    s_id = '29e9b83b-61b4-4c41-886d-0894bdf32184'
    assistant_id = 'b7f18534-25cc-43fa-98d3-277629137832'
    ...
    Output2 = response1['output']['generic'][0]['text']
    api = IAMAuthenticator("P4oALmMBfEquxYbT4Z-QI1yGTjb6EOL-jyVWYcD-Suwg")
    text_2_speech = TextToSpeechV1(authenticator=api)
    Voiceoutput = "Welcome.mp3"
    text_2_speech.set_service_url(
        "https://api.us-south.text-to-speech.watson.cloud.ibm.com/instances/5c53655b-8a2a-4133-a9dc-00e3a8407dcc")
    with open(Voiceoutput, "wb") as audiofile:
        audiofile.write(text_2_speech.synthesize(Output2, accept="audio/mp3").get_result().content)

    audiofile.close()
    playsound(Voiceoutput)
    condition = True
    global Counter
    Counter = 0
    while condition:
        counterStr1= str(Counter)
        ...
        ws.run_forever()
        output1="output"+counterStr1+".txt"
        text_file = open(output1, "w")
        n = text_file.write(final_string+counterStr1)
        text_file.close()
        ...
        Output2 = response['output']['generic'][0]['text']

        api = IAMAuthenticator("P4oALmMBfEquxYbT4Z-QI1yGTjb6EOL-jyVWYcD-Suwg")
        text_2_speech = TextToSpeechV1(authenticator=api)
        print(counterStr1)
        Voiceoutput="Voiceoutput"+counterStr1+".mp3"
        print(Voiceoutput)
        text_2_speech.set_service_url(
            "https://api.us-south.text-to-speech.watson.cloud.ibm.com/instances/5c53655b-8a2a-4133-a9dc-00e3a8407dcc")
        with open(Voiceoutput, "wb") as audiofile:
